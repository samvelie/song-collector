...
router.post('/addImageNewProcess/:id', function(req, res) {
  var userId = req.userInfo.id; // will become user id pulled from decoder token
  ...
  pool.connect(function(err, client, done) {
    if(err) {
      ...
      client.query('WITH new_image_id AS (INSERT INTO images (image_file_name, image_url, is_notation) VALUES ($1, $2, $3) RETURNING id) INSERT INTO images_songs (image_id, song_id, user_id) VALUES ((SELECT id FROM new_image_id), $4, $5);', [imageObject.fileName, imageObject.url, imageObject.isNotation, songId, userId], function(err, result) {
        done();
        if(err) {
          console.log('error making database query: ', err);
          res.sendStatus(500);
